...
const SoPDashboard: React.FC<SoPDashboardProps> = ({ sopVersions }) => {
  const [versions, setVersions] = useState<SoPVersion[]>(sopVersions);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      formData.append("sopFile", selectedFile);

      // Upload to server
      const response = await fetch("http://localhost:3000/api/sop/upload", {
        method: "POST",
        body: formData,
      });
      ...
      const fileUrl =
        result.data?.fileUrl ||
        `http://localhost:3000/api/sop/files/${selectedFile.name.replace(
          /\s+/g,
          "_"
        )}`;
      const score = result.data?.analysis?.score || 0;
      const feedback = result.data?.analysis?.feedback || [
        "Your SoP has been uploaded successfully.",
        "We are analyzing your SoP for improvement opportunities.",
        "Consider adding more details to your research interests.",
      ];
      ...
      toast({
        ...
        description: `Your SoP has been uploaded and scored ${score}/100.`,
      });
    } catch (error) {
  ...
  const handleDownload = (version: SoPVersion) => {
  ...
  const handleFormatting = () => {
    ...
    toast({
      ...
      description: "Auto-formatting to program requirements would be implemented here.",
    });
  };
  ...
  const handleSendToMentor = async (version: SoPVersion) => {
    setIsSending(true);
    try {
      const result = await sendDocumentToMentor(version, "sop");

      if (result.success) {
        toast({
          title: "Sent to mentor",
          description: "Your SoP has been sent to your mentor for review.",
        });
      } else {
        throw new Error(result.message);
      }
    } catch (error) {
      console.error("Error sending SoP to mentor:", error);
      toast({
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Statement of Purpose Dashboard</h3>
        <div className="flex space-x-2">
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Upload Statement of Purpose</DialogTitle>
                <DialogDescription>
                  Upload your current SoP. Supported formats: PDF, DOCX.
                </DialogDescription>
              </DialogHeader>
              <div className="py-6">
                <div
              ...
              <DialogFooter className="flex space-x-2">
                <Button variant="outline" onClick={handleFormatting}>
                  Auto-Format to Program Requirements
                </Button>
                <Button
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Create New SoP Version</DialogTitle>
                <DialogDescription>
              ...
              <div className="py-4">
                ...
                <p className="text-center text-muted-foreground">
                  SoP creation form would go here
                </p>
              </div>
            </DialogContent>
          </Dialog>
      ...
      {versions.length === 0 ? (
        <Card className="border-dashed border-2">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-lg font-medium mb-2">No SoP versions yet</h3>
                ...
                {version.feedback && version.feedback.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="text-xs text-muted-foreground mt-1 flex justify-between">
                      <span>AI-generated feedback based on SoP analysis</span>
                      {version.score !== undefined && (
                        <span
                          className={`font-medium ${scoreColor(version.score)}`}
                        >
